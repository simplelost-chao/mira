...
// ── Summary (Tab 1): B-layout — hero + 3-col grid ────────────────────────────
function renderSummary() {{
  ...
  let html = `<div style="display:flex;align-items:flex-start;justify-content:space-between;margin-bottom:20px">
    ...
    ${{ca ? `<div style="text-align:right;font-size:11px;color:var(--muted)">Claude ${{caLastStr}}</div>` : ''}}
  </div>`;
  ...
  // Git card (1 col)
  html += `<div style="background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.07);border-radius:8px;padding:14px 16px">
    <div style="font-size:9px;font-weight:700;letter-spacing:2px;text-transform:uppercase;color:var(--muted);margin-bottom:10px">代码库</div>
    <div style="font-size:22px;font-weight:700;color:var(--gold);margin-bottom:3px">${{git.monthly_commits || 0}}</div>
    <div style="font-size:10px;color:var(--sub);margin-bottom:10px">本月提交</div>
    <div style="font-size:11px;color:var(--sub);display:flex;flex-direction:column;gap:3px">
      ${{git.branch ? `<span>${{escHtml(git.branch)}}${{git.commit_hash ? ' · '+git.commit_hash.slice(0,7) : ''}}</span>` : ''}}
      ${{(p.loc||{{}}).code_lines ? `<span style="color:var(--muted)">${{((p.loc.code_lines)/1000).toFixed(1)}}k 代码行 · ${{p.loc.file_count}} 文件</span>` : ''}}
      ${{(git.dirty_files||[]).length ? `<span style="color:var(--orange)">${{git.dirty_files.length}} 未提交文件</span>` : ''}}
    </div>
  </div>`;

  // Claude card (2 col) with horizontal stats + spark
  if (ca) {{
    ...
    const todoHtml = todos.map(t => {{
      ...
      const c = s==='completed'?'var(--green)':s==='in_progress'?'var(--gold)':'var(--border)';
      return `<div style="display:flex;align-items:flex-start;gap:5px;font-size:10px">
        <div style="width:5px;height:5px;border-radius:50%;background:${{c}};flex-shrink:0;margin-top:3px"></div>
        <span style="color:${{s==='completed'?'var(--green)':s==='in_progress'?'var(--gold)':'var(--sub)'}}">${{escHtml(t.content||'')}}</span>
      </div>`;
    }}).join('');

    html += `<div style="background:rgba(79,70,229,.04);border:1px solid rgba(79,70,229,.15);border-radius:8px;padding:14px 16px">
      <div style="font-size:9px;font-weight:700;letter-spacing:2px;text-transform:uppercase;color:var(--muted);margin-bottom:10px">Claude</div>
      <div style="display:flex;gap:16px;align-items:flex-end">
        <div style="display:flex;gap:12px;flex-wrap:wrap">
          <div><div style="font-size:18px;font-weight:700;color:var(--purple)">$${{(ca.estimated_cost_usd||0).toFixed(1)}}</div><div style="font-size:9px;color:var(--muted);margin-top:2px">累计花费</div></div>
          <div><div style="font-size:18px;font-weight:700;color:var(--purple)">${{ca.session_count_30d||0}}</div><div style="font-size:9px;color:var(--muted);margin-top:2px">30天会话</div></div>
          <div><div style="font-size:18px;font-weight:700;color:var(--purple)">${{fmtTok(ca.output_tokens||0)}}</div><div style="font-size:9px;color:var(--muted);margin-top:2px">输出</div></div>
          <div><div style="font-size:18px;font-weight:700;color:var(--purple)">${{fmtTok(ca.cache_read_tokens||0)}}</div><div style="font-size:9px;color:var(--muted);margin-top:2px">Cache读</div></div>
        </div>
        <div class="cl-spark" style="flex:1;min-width:80px">${{sparkBars}}</div>
      </div>
      ${{todoHtml ? `<div style="margin-top:10px;display:flex;flex-direction:column;gap:4px">${{todoHtml}}</div>` : ''}}
    </div>`;
  }} else {{
    html += `<div style="background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.07);border-radius:8px;padding:14px 16px;display:flex;align-items:center;justify-content:center;color:var(--muted);font-size:12px">暂无 Claude 数据</div>`;
  }}

  // tech stack + features continue in same flex column
  ...
  if (techStack.length) {{
    ...
    for (const [cat, items] of Object.entries(groups)) {{
      ...
      techHtml += `<div style="margin-bottom:8px">
        <div style="font-size:9px;font-weight:700;letter-spacing:1.5px;text-transform:uppercase;color:var(--muted);margin-bottom:4px">${{glabels[cat]}}</div>
    ...
    html += `<div style="background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.07);border-radius:8px;padding:14px 16px">
      <div style="font-size:9px;font-weight:700;letter-spacing:2px;text-transform:uppercase;color:var(--muted);margin-bottom:10px">技术栈</div>
      ${{techHtml}}
    </div>`;
  }}
  ...
  if (features.length) {{
    ...
    const todo = features.filter(f => f.implemented === false);
    const pct = Math.round(done.length / features.length * 100);
    let featHtml = `<div style="display:flex;align-items:center;gap:10px;margin-bottom:12px">
      <div style="flex:1;height:3px;background:rgba(255,255,255,.07);border-radius:2px;overflow:hidden">
        <div style="width:${{pct}}%;height:100%;background:linear-gradient(90deg,#4f46e5,#22c55e);border-radius:2px"></div>
      </div>
      <span style="font-size:10px;color:var(--sub);white-space:nowrap">${{done.length}}/${{features.length}} (${{pct}}%)</span>
    </div>
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:2px 16px">`;
    for (const f of done.slice(0,8)) {{
      featHtml += `<div style="display:flex;align-items:flex-start;gap:6px;font-size:11px;padding:3px 0">
        <span style="color:var(--green);flex-shrink:0">✓</span>
        <span style="color:var(--sub)">${{escHtml(f.text||'')}}</span>
      </div>`;
    }}
    for (const f of todo.slice(0,4)) {{
      featHtml += `<div style="display:flex;align-items:flex-start;gap:6px;font-size:11px;padding:3px 0">
        <span style="color:var(--border);flex-shrink:0">○</span>
        <span style="color:var(--muted)">${{escHtml(f.text||'')}}</span>
      </div>`;
    }}
    featHtml += `</div>`;
    html += `<div style="background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.07);border-radius:8px;padding:14px 16px">
      <div style="font-size:9px;font-weight:700;letter-spacing:2px;text-transform:uppercase;color:var(--muted);margin-bottom:10px">功能进度</div>
      ${{featHtml}}
    </div>`;
  }}

  html += `</div>`;  // end cards stack
  el.innerHTML = html;
  summaryLoaded = true;
}}
